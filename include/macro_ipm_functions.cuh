...
\param p -- macro-model derivatives of the potential
...
\return w = z - alpha_macro - alpha_star - alpha_local - alpha_smooth
******************************************************************************/
template <typename T>
__device__ Complex<T> complex_image_to_source(Complex<T> z, potential<T> p, 
	T theta, star<T>* stars, T kappastar, TreeNode<T>* node,
	int rectangular, Complex<T> corner, int approx, int taylor_smooth)
{
	Complex<T> a_macro = alpha_macro<T>(z, p);
	...
	/******************************************************************************
	z - alpha_macro - alpha_star - alpha_local - alpha_smooth
	******************************************************************************/
...
******************************************************************************/
template <typename T>
__device__ T magnification(Complex<T> z, potential<T> p, 
	T theta, star<T>* stars, T kappastar, TreeNode<T>* node,
	int rectangular, Complex<T> corner, int approx, int taylor_smooth)
{
	T d_a_macro_d_z = d_alpha_macro_d_z<T>(z, p);
	Complex<T> d_a_macro_d_zbar = d_alpha_macro_d_zbar<T>(z, p);
	...
	T d_w_d_z = 1 - d_a_macro_d_z - d_a_smooth_d_z;
	Complex<T> d_w_d_zbar = -d_a_macro_d_zbar - d_a_star_d_zbar - d_a_local_d_zbar - d_a_smooth_d_zbar;
...
******************************************************************************/
template <typename T>
__global__ void shoot_cells_kernel(potential<T> p, T theta, star<T>* stars, T kappastar, TreeNode<T>* root,
	int rectangular, Complex<T> corner, int approx, int taylor_smooth,
...
#pragma unroll
			for (int a = 0; a < 4; a++)
			{
				...
				y[a] = complex_image_to_source(x[a], p, theta, stars, kappastar, node, rectangular, corner, approx, taylor_smooth);
				/******************************************************************************
